...
function addToolbarAndLink(component) {
  component.em.get('Commands').run(LINK_COMMAND, {
    component
  });
...
export default class DynamicContentDomComponents {
  static addDynamicContentType(editor) {
    ...
    if (isMjml) {
      ...
      const baseModel = baseType.model; // Keep style-default from mj-text so padding etc. render correctly via MJML
      ...
      const view = {
        ...
        onActive() {
          this.em.get('Commands').run(OPEN_COMMAND, {
            target: this.model
          });
        }
